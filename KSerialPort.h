...
public:
...
#endif
	int read_some(std::vector<uint8_t> &buffers, boost::system::error_code& ec) {
		return mport.read_some(boost::asio::buffer(buffers, buffers.size()), ec);
	}
	int read_some(uint8_t *buffers, int len, boost::system::error_code& ec) {
	...
	int read(uint8_t *buffers, int len, boost::system::error_code& ec) {
		size_t numread = 0;
	...
	int readLine(uint8_t *buffers, int len, boost::system::error_code& ec) {
		size_t numread = 0;
		boost::asio::deadline_timer timeout(io_srv);
		while (numread < len) {
			size_t async_numread = 0;
			...
			timeout.expires_from_now(boost::posix_time::milliseconds(100));
			timeout.async_wait(boost::bind(&KSerialPort::timeout_callback, this, boost::ref(mport), boost::asio::placeholders::error));
			boost::asio::async_read(mport, boost::asio::buffer(&buffers[numread], len-numread), boost::asio::transfer_at_least(1), boost::bind(&KSerialPort::async_read_callback, this, boost::ref(timeout), boost::ref(async_numread), _1, _2));
			io_srv.run();
			if (async_numread == 0) {
				break;
			}
			numread += async_numread;
			if (buffers[numread-1] == '\n') {
				break;
			}
		}
		io_srv.reset();
	...
	int write_some(const std::vector<uint8_t> &buffers, boost::system::error_code& ec) {
		return mport.write_some(boost::asio::buffer(buffers, buffers.size()), ec);
	}
	int write_some(const uint8_t *buffers, int len, boost::system::error_code& ec) {
